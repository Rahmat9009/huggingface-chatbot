...
export async function createConversation(userId: number, title: string = "New Chat") {
  ...
  // Return the inserted conversation
  const inserted = await db.select().from(conversations).where(eq(conversations.userId, userId)).orderBy(desc(conversations.createdAt)).limit(1);
  return inserted[0];
}
